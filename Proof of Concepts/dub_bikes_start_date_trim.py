...
SELECTED_STATIONS = [19, 96, 10]


def plot_station_data(time_between_start_end_days, y_available_bikes, station_id):
    # plot number of available bikes at station id vs number of days
    plt.rc("font", size=18)
    plt.rcParams["figure.constrained_layout.use"] = True
    plt.figure(figsize=(8, 8), dpi=80)
    # plt.plot(time_between_start_end_days, y_available_bikes, '-o')
    plt.scatter(time_between_start_end_days, y_available_bikes, color="red", marker=".")
...
def plot_preds(time_between_start_end_days, y_available_bikes, time_preds_days, y_pred, station_id):
    plt.scatter(time_between_start_end_days, y_available_bikes, color="black")
...
def predidct_available_bikes_seasonality(
    ...
    time_for_each_prediction_in_days = time_between_start_end_days[
        lag * seasonality + q_step_ahead :: stride
    ]
    ...
    mse = mean_squared_error(
        ...
        model.predict(XX_input_features[test]),
...
def test_various_seasonaliy_preds(
    y_available_bikes, time_between_start_end_days, time_sampling_interval_dt, station_id
):
    plot = True
    ...
    predidct_available_bikes_seasonality(
        ...
        time_between_start_end_days=time_between_start_end_days,
...
def exam_2021(df_station, station_id):
    ...
    # extract data between start date (wed the 29th of Jan 2020) to end date in df(Wed 1st Apr 2020)
    time_start_seconds = pd.DatetimeIndex([start_date]).astype(np.int64)/1000000000
    time_start_seconds = time_start_seconds.to_numpy()
    time_start_seconds= time_start_seconds[0]


    time_between_start_end_seconds = np.extract([(time_full_seconds>=time_start_seconds)], time_full_seconds)
    time_between_start_end_days = (
        (time_between_start_end_seconds - time_between_start_end_seconds[0]) / 60 / 60 / 24
    )  # convert timestamp to days
    y_available_bikes = np.extract([time_full_seconds>=time_start_seconds], df_station.iloc[:, 6]).astype(
        np.int64
    )

    # plot extracted data
    plot_station_data(time_between_start_end_days, y_available_bikes, station_id)
    ...
    plot_preds(time_between_start_end_days, y_available_bikes, time_preds_days, y_pred, station_id)
